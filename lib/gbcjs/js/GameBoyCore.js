...
//FIXME : Saves interrupt state after.
GameBoyCore.SCRAMBLE_get_state = function(keys){
	var intr = {};
	intr.interruptsRequested = this.interruptsRequested;
	intr.interruptsEnabled = this.interruptsEnabled;
	intr.mem_FF00 = this.memory[0xFF00];
	return intr;
}
GameBoyCore.SCRAMBLE_sync_load_state = function(dat){
	this.memory[0xFF00] = dat.mem_FF00;
	this.interruptsEnabled = dat.interruptsEnabled;
	this.interruptsRequested = this.interruptsRequested;
...
GameBoyCore.prototype.run = function () {
	...
	console.log("S:", this.programCounter, this.serialTimer, this.serialShiftTimer, this.CPUTicks,this.idx);
	...
	//FIXME
	var opcodeToExecute = this.memoryReader[this.programCounter](this, this.programCounter);
	console.log("F:", this.programCounter, this.serialTimer, this.serialShiftTimer, this.CPUTicks, this.idx);
	this.idx+=1;
}
...
GameBoyCore.prototype.launchIRQ = function () {
	...
	do {
		...
		if ((testbit & this.IRQLineMatched) == testbit) {
			...
			//FIXME
			console.log("IRQ:", testbit, this.stackPointer,  this.programCounter);
